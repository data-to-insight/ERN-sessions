...
def ethnic_hist(df):
    fig = px.histogram(df, "ETHNIC", title="Breakdown of ethnicities selected")
    return fig
...
if file:
    ...
    ref_date = st.sidebar.date_input(
        label="Set reference date", value=pd.to_datetime("today")
    )
    ...
    if len(age_bucket_range) == 4:
        ...
        df = df[
            (ref_date - pd.DateOffset(years=age_range[0]) >= df["DOB"])
